# ARIA 0.6.2-draft — Red Team Red Signals
> All critical findings from all 28 panelists. For per-panelist synthesis (green + red together), see RED-TEAM-SYNTHESIS.md. For ranked priority, see the Ranked Fix List at the bottom of this document.
...
*(Red signals from ARIA toward HCS — gaps that affect companion deployments)*
...
HCS → ARIA implications (work product for HCS repo — review with Founder before action):
...
2. HCS IHS authentication must complete BEFORE ARIA `prefill()` is called. ARIA's design allows this (auth is above ARIA), but the spec should acknowledge pre-call identity verification as a valid companion deployment pattern.
